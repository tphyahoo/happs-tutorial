...
-- taken from fgl
-- tweaked by Thomas Hartman for HAppS Tutorial, to be read/showable.

module SerializeableFiniteMap(
    -- * Type
    FiniteMap(..),
    -- * Operations
    emptyFM,
...
-- copy/pasted from Data.Graph.Inductive, and slightly modified to be read/showable/serializeable using basic deriving
...
-- out of curiosity:
-- can we get away with not deriving Data,Typeable, and still have happs serialization machinery work?
...
-- just use deriving show in dtaa declaration, then we get read . show compatibility
-- and serialization works, and graphs can be used as data stores
{-                
...
--fmFromList :: Ord a => [(a,b)] -> FiniteMap a b 
-- copy/paste/tweak from Data.Map. Bad code reuse smell.
{--------------------------------------------------------------------
  Lists 
  use [foldlStrict] to reduce demand on the control-stack
--------------------------------------------------------------------}
-- | /O(n*log n)/. Build a map from a list of key\/value pairs. See also 'fromAscList'.
fmFromList :: Ord k => [(k,a)] -> FiniteMap k a 
fmFromList xs       
  = foldl' ins Empty xs
  where
    ins t (k,x) = addToFM t k x 
